...
Private mode removes Anchor-generated diagnostic details, instruction-name
logs, panic text, and nonessential security metadata. Numeric error codes,
IDLs, account and instruction layouts, explicit application logs, and events
are unchanged.

Private builds also ask rustc to omit source locations, debug information, and
ordinary symbols. Existing Cargo features, flags, and compiler wrappers are
preserved.

Use `--no-private` to override `[build] private = true`. A program that enables
the `anchor-lang/private-program` Cargo feature remains private because manifest
features cannot be disabled by the CLI.

Private mode is incompatible with `cargo build-sbf --debug`. Size optimization
remains separate and can still be selected explicitly with
`anchor build --private -- --optimize-size`.

After compilation, Anchor scrubs recognized source-path spans and audits the
ELF before replacing the output. The build fails if debug or ordinary symbol
sections, source paths, Anchor diagnostic templates, unexpected exports, or
excess security metadata remain. It reports only aggregate string counts and
does not print retained application strings.

See the fixture's
[Markdown comparison](../../../../tests/private-program/reports/private-program-comparison.md)
or [static HTML comparison](../../../../tests/private-program/reports/private-program-comparison.html)
for normal and private artifact results.
